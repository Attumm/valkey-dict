...
def run_load_test(dataset, times=1, use_compression=False):
    ...
    valkey_dict.clear()
    initial_size = valkey_dict.valkey.info(section="memory")["used_memory"]
    if use_compression:
        valkey_dict.extends_type(dict, encode_dict, decode_dict)
    ...
    final_size = valkey_dict.valkey.info(section="memory")["used_memory"]
    valkey_dict.clear()
